  ...
  <div class="min-h-screen flex bg-zinc-900">
    ...
    <aside class="w-2/6 h-screen bg-zinc-700 flex flex-col justify-start">
      ...
      <main class="grid grid-cols-2 grid-rows-3 gap-2 p-4 w-full h-5/6">
        <div :key="index" class="bg-zinc-900 w-full h-full rounded-xl p-2">
          <img src="../img/buyBee.png" class="w-full h-2/3 rounded-lg " />
          <div class="h-1/3 w-full flex items-center justify-center">
            <button @click="buyBee(index)" class="text-zinc-50 text-md hover:text-zinc-900 hover:bg-zinc-300 font-semibold bg-zinc-700 rounded-md px-6 py-2 select-none"><p class="align-text-bottom">100<span class="align-text-top">🍯</span></p></button>
          </div>
        </div>
        ...
        <div :key="index" class="bg-zinc-900 w-full h-full rounded-xl p-2">
          <img src="../img/buyBee.png" class="w-full h-2/3 rounded-lg " />
          <div class="h-1/3 w-full flex items-center justify-center">
            <button @click="buyBee(index)" class="text-zinc-50 text-md hover:text-zinc-900 hover:bg-zinc-300 font-semibold bg-zinc-700 rounded-md px-6 py-2 select-none"><p class="align-text-bottom">500<span class="align-text-top">🍯</span></p></button>
          </div>
        </div>
        ...
        <div :key="index" class="bg-zinc-900 w-full h-full rounded-xl p-2">
          <img src="../img/buyBee.png" class="w-full h-2/3 rounded-lg " />
          <div class="h-1/3 w-full flex items-center justify-center">
            <button @click="buyBee(index)" class="text-zinc-50 text-md hover:text-zinc-900 hover:bg-zinc-300 font-semibold bg-zinc-700 rounded-md px-6 py-2 select-none"><p class="align-text-bottom">???<span class="align-text-top">🍯</span></p></button>
          </div>
        </div>
...
const count = ref(0);
const beeNumber = ref(0);
const maszyna = ref(0); // Use ref for reactive properties
const inputValue = ref('');
const baseBeeCost = 100; // Initial cost of a Bee
const bees = []; // Array to hold the costs of each bee upgrade
const ones = ref([])

const loadCountFromCookies = () => {
  const savedCount = VueCookies.get('count');
  const savedBeeNumber = VueCookies.get('beeNumber');
  if (savedCount) {
    count.value = parseInt(savedCount, 10);
  }
  if (savedBeeNumber) {
    beeNumber.value = parseInt(savedBeeNumber, 10);
    calculateMaszyna();
  }
};
...
const saveCountToCookies = () => {
  ...
  VueCookies.set('beeNumber', beeNumber.value, '1d');
};
...
const buyBee = (index) => {
  const cost = baseBeeCost + beeNumber.value * 100; // Calculate dynamic cost
  if (count.value >= cost) {
    count.value -= cost;
    beeNumber.value += 1;
    calculateMaszyna();
  } else {
    alert("You don't have enough count to buy this Bee!");
  }
};

const addToCount = () => {
  const parsedValue = parseInt(inputValue.value, 10);
  if (!isNaN(parsedValue)) {
    count.value += parsedValue;
    saveCountToCookies();
    inputValue.value = '';
  } else {
...
const calculateMaszyna = () => {
  maszyna.value = beeNumber.value; // Update maszyna based on beeNumber
};

setInterval(() => {
  count.value += maszyna.value;
}, 1000);
